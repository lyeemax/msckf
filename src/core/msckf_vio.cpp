...
bool MsckfVio::loadParameters(Stereo_camera_config_t& params)
{
    ...
    // The initial orientation and position will be set to the origin
...
void MsckfVio::stateAugmentation(const uint64_t& time)
{
    ...
    Eigen::Matrix3d R_w_i = quaternionToRotation(
        state_server.imu_state.orientation);
    Eigen::Matrix3d R_w_c = R_i_c * R_w_i;  //aka. R_c_w=R_c_i*R_i_w
    Eigen::Vector3d t_c_w = state_server.imu_state.position +// aka. t_w_c=t_w_i+R_w_i*t_i_c
